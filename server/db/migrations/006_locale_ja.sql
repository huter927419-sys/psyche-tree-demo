-- Japanese locale + trilingual oracle readings

ALTER TABLE book_assessments ADD COLUMN mystical_reading_ja TEXT;
ALTER TABLE book_assessments ADD COLUMN mystical_reading_source_ja TEXT CHECK (
  mystical_reading_source_ja IS NULL OR mystical_reading_source_ja IN ('deepseek', 'fallback')
);

ALTER TABLE journeys ADD COLUMN holistic_reading_ja TEXT;
ALTER TABLE journeys ADD COLUMN holistic_reading_source_ja TEXT CHECK (
  holistic_reading_source_ja IS NULL OR holistic_reading_source_ja IN ('deepseek', 'fallback')
);
ALTER TABLE journeys ADD COLUMN holistic_prompt_input_ja TEXT;

-- SQLite cannot alter CHECK constraints; rebuild tables to allow locale = 'ja'.
...
CREATE TABLE book_assessments_new (
  ...
  mystical_reading_locale TEXT CHECK (
    mystical_reading_locale IS NULL OR mystical_reading_locale IN ('zh', 'en', 'ja')
  ),
...
INSERT INTO book_assessments_new SELECT
  ...
  mystical_reading_zh, mystical_reading_en, mystical_reading_ja,
  mystical_reading_source_zh, mystical_reading_source_en, mystical_reading_source_ja,
...
CREATE TABLE journeys_new (
  ...
  holistic_reading_locale TEXT CHECK (
    holistic_reading_locale IS NULL OR holistic_reading_locale IN ('zh', 'en', 'ja')
  ),
...
INSERT INTO journeys_new SELECT
  ...
  holistic_reading_ja, holistic_reading_source_zh, holistic_reading_source_en,
  holistic_reading_source_ja, holistic_prompt_input_zh, holistic_prompt_input_en,
  holistic_prompt_input_ja
FROM journeys;
